...
需求基准：superstorm-registry · 共 13 面，盘问 3 面 · 实测 3 问 · 旅程 4 条，盘问 1 条 · 操作面 97 个，裁决触及 11 个

## 总览

实证完成：1（运行时 1 · 代码 0 · 台账 0） · 缺口：2 · 跑偏：0 · 无法自证：0
旅程裁决：实证完成：0 · 缺口：0 · 跑偏：1 · 无法自证：0
...
取证时间：首问 2026-09-16T18:47:01+09:00 · 末问 2026-09-16T21:09:41+09:00 · 跨 142 分钟
相邻 runtime 问间隔不足 60 秒，请核对是否真实取证：聊天页、状态栏、标签条、会话侧栏、作业视图、助手行这六个面，在 600/640/899/1100/1300/1400/1920 七档窗口宽度下还成立吗？ → 视频页、音乐页、资源页、素材库、设置抽屉这五个面，在 600/640/899/1100/1300/1400/1920 七档窗口宽度下还成立吗？（0 秒）

## 规则一致性（规则 ↔ 规则；候选不计为产品缺陷）
历史台账未做规则对比；不追溯拒收原裁决。

## 需求覆盖（基准 92 条，有裁决 11 条，无裁决 81 条）
...
- R-ui-02 聊天面板：模型选择、加载/卸载、流式回复、思考过程可展开、左侧会话列表（新建/切换/重命名/删除）。 — 缺口（聊天页、状态栏、标签条、会话侧栏、作业视图、助手行这六个面，在 600/640/899/1100/1300/1400/1920 七档窗口宽度下还成立吗？）
...
- R-ui-07 常驻状态条：真实内存（不是写死的 128）、内存里是谁、媒体是否在跑、现在能不能开下一件重活。 — 缺口（聊天页、状态栏、标签条、会话侧栏、作业视图、助手行这六个面，在 600/640/899/1100/1300/1400/1920 七档窗口宽度下还成立吗？）
- R-ui-10 设置界面：对外 API 的开关、主机、端口，并显示可复制的 base URL — 缺口（视频页、音乐页、资源页、素材库、设置抽屉这五个面，在 600/640/899/1100/1300/1400/1920 七档窗口宽度下还成立吗？）
- R-shell-04 退出时终止内嵌服务；不得留下孤儿进程（退出后台面端口不再有监听）。 — 跑偏（J2 走得通吗？）
- R-shell-05 Dock 图标存在，App 可被正常激活与切换（不是 LSUIElement）。 — 跑偏（J2 走得通吗？）
- R-foundation-01 所有路径由本模块单点解析，区分「只读 bundle 资源根」与「可写用户数据根」； — 无裁决（面 F1 未盘问；面 F12 未盘问）
- R-foundation-02 配置持久化到 `<data root>/config.json`，字段至少含： — 无裁决（面 F1 未盘问）
- R-foundation-03 首运语义：无配置或 `first_run_done` 为假时，`api:readConfig` 必须能表达 needs-setup 状态； — 无裁决（面 F1 未盘问）
- R-foundation-04 收编既有权重：给定一个 legacy 根（如 `~/LocalModelDesk`）， — 无裁决（面 F1 未盘问）
- R-foundation-05 服务在 `mlx-h3`、内嵌 venv、模型目录任一缺失时仍能启动并提供状态接口； — 无裁决（面 F1 未盘问）
- R-foundation-06 配置写入原子：先写同目录临时文件再 `os.replace`；写到一半崩掉不得留下损坏的 `config.json`。 — 无裁决（面 F1 未盘问）
- R-resources-01 单一目录定义，供服务端、前端、以及任何 CLI 入口共同消费； — 无裁决（面 F2 未盘问）
- R-resources-02 完整度以 HF 仓库文件清单为准：取得每个期望文件的路径与字节数， — 无裁决（面 F2 未盘问）
- R-resources-03 每项报出 `present` | `partial` | `missing`；`partial` 必须带完成百分比 — 无裁决（面 F2 未盘问）
- R-resources-04 报出每项实际占盘字节数，以及 models 所在卷的剩余空间。 — 无裁决（面 F2 未盘问）
- R-resources-05 对某一目录项发起下载；中断后再次发起必须续传，已完整的文件不得重下。 — 无裁决（面 F2 未盘问；面 F13 未盘问）
...
- R-arbiter-06 暴露单一台面状态对象：内存里是谁、媒体是否在跑、现在能不能开下一件重活， — 无裁决（面 F3 未盘问）
- R-arbiter-07 加载某模型前比对其体积与当前可用内存，装不下要在加载前给出警告（不是加载失败后才说）。 — 无裁决（面 F3 未盘问）
- R-llm-01 按目录 key 加载聊天模型：先向 `arbiter` 申请许可，再启动 mlx-lm 服务， — 无裁决（面 F4 已盘问但无裁决引用它）
...
- R-llm-04 加载失败必须给出原因（mlx-lm 日志尾部），不得只报一个「加载超时」。 — 无裁决（面 F4 已盘问但无裁决引用它）
...
- R-gateway-01 `GET /v1/models`：OpenAI 格式，列出当前可服务的模型。没有模型驻留时返回空列表而非报错。 — 无裁决（面 F5 未盘问）
- R-gateway-02 `POST /v1/chat/completions` 非流式：OpenAI 请求与响应形状，含 `usage`、`finish_reason`。 — 无裁决（面 F5 未盘问）
- R-gateway-03 同端点流式：SSE，`data:` 分块与 OpenAI 增量格式一致，以 `data: [DONE]` 结束。 — 无裁决（面 F5 未盘问）
- R-gateway-04 `POST /v1/messages` 非流式：Anthropic 请求形状（`model`/`max_tokens`/`messages`/`system`） — 无裁决（面 F5 未盘问）
- R-gateway-05 同端点流式：发出 `message_start`、`content_block_start`、`content_block_delta`、 — 无裁决（面 F5 未盘问）
- R-gateway-06 Anthropic 的 `system`（字符串或块数组）、多轮 `messages`、`stop_reason` — 无裁决（面 F5 未盘问）
- R-gateway-07 绑定 `0.0.0.0` 于配置端口，使同网络其他设备可达；开关关闭时不监听。 — 无裁决（面 F5 未盘问）
- R-gateway-08 无模型驻留或媒体作业进行中：返回 `503`，带 `Retry-After` 头与机器可读原因码。 — 无裁决（面 F5 未盘问；面 F13 未盘问）
- R-gateway-09 错误按各自方言的原生错误信封返回（OpenAI 的 `{"error":{...}}`； — 无裁决（面 F5 未盘问）
- R-gateway-10 请求里的 `model` 既接受目录 key，也接受当前加载模型的标识符； — 无裁决（面 F5 未盘问）
...
- R-library-01 列出成品（视频/音频），带时间、类别、字节数。列表来源同时覆盖历史记录与磁盘上的孤儿文件 — 无裁决（面 F7 未盘问；面 F12 未盘问）
...
- R-shell-01 App 从 `Contents/Resources/` 内的内嵌运行时启动服务； — 无裁决（面 F9 已盘问但无裁决引用它）
...
- R-shell-06 App 不注册任何登录项、不写任何 LaunchAgent plist。 — 无裁决（面 F9 已盘问但无裁决引用它）
- R-shell-07 首运弹出 models 目录选择（`NSOpenPanel`），并可选择收编既有目录树。 — 无裁决（面 F9 已盘问但无裁决引用它）
...
### 聊天与模型加载（F4）— 1 问中 1 问实证通过 · 操作面 8 个，裁决触及 4 个，未触及：S16 加载聊天模型、S17 卸载/取消加载、S21 提示词助手（看手气/优化）、S50 mlx-lm 子进程后端契约（出站）
...
### 原生外壳：窗口、菜单、状态项、错误页、生命周期（F9）— 0 问中 0 问实证通过 · 操作面 15 个，裁决触及 4 个，未触及：S51 应用主菜单（关于 / 设置…⌘, / 隐藏 ⌘H / 隐藏其他 ⌥⌘H / 全部显示 / 退出 ⌘Q）、S52 文件菜单（关闭窗口 ⌘W）、S53 编辑菜单（撤销/重做/剪切/复制/粘贴/粘贴并匹配样式/删除/全选）、S54 窗口菜单（最小化 ⌘M / 进入全屏幕 ⌃⌘F）、S55 ⌘, 本地键盘监听（WKWebView 吞掉键等价物的补偿）、S56 菜单栏状态项与其菜单（状态行/内存行/打开窗口/打开成品目录/设置…/退出）、S57 WKWebView 上下文菜单（重新载入本地化）、S63 退出时按进程组同步收割子进程并核验端口已释放、S66 父进程看门狗（LMD_PARENT_PID 消失即退出）、S67 SIGTERM/SIGINT 优雅退出与自成进程组、D4 死契约：PortGuard.family(matching:)
...
## 未盘问声明（按风险排序）
- 旅程 第一次打开 LocalModelDesk、机器上已有若干 MLX 模型目录的人 · 全新数据根（没有 config.json），从 app 启动 · 台面可用，且模型目录指向他自己的目录（J1）— 未盘问，不计入任何完成度 · 风险 high：R-foundation-03/05 的首运语义是所有人的第一分钟；走不通等于产品不可用
- 旅程 在外接显示器与笔记本屏之间来回的人 · 台面开着，逐档改窗口宽度（900 / 899 两侧 / 1100 两侧 / 1400 之上 / 1920） · 每个宽度下界面都可读可操作（J4）— 未盘问，不计入任何完成度 · 风险 high：R-ui-06/07 的响应式；上一轮零视觉裁决，这条正是当时没人验的那类
- 旅程 想找刚生成的文件的人 · app 在跑但主窗口关着，只剩菜单栏图标 · 不开主窗口就在访达里打开成品目录（J3）— 未盘问，不计入任何完成度 · 风险 medium：R-shell-06/07 的常驻入口；坏了只是绕路，不是断路
- 首次运行与配置落地（F1）— 未盘问，不计入任何完成度 · 未评估风险
- 模型资源：校验、下载续传、删除（F2）— 未盘问，不计入任何完成度 · 未评估风险
- 台面互斥与内存仲裁（F3）— 未盘问，不计入任何完成度 · 未评估风险
- 对外网关（F5）— 未盘问，不计入任何完成度 · 未评估风险
...
- [F1] POST /api/first-run 的 models_root 传一个非字符串（如数字或对象），客户端拿到的是 400 参数错误还是 500 internal？ — 泄漏点：P2 枚举官读码指出 paths.py:22 Path(value) 的 TypeError 无人捕获（未实测）
