...
- 复制锁定 CPython 后必须删除其 `__pycache__`/`.pyc`，将 `libpython3.13.dylib` install-id 改为 bundle 相对值，并净化 `_sysconfigdata` 与依赖源码注释中的构建机绝对串；之后再签名。源前缀必须由 `SRC_PY` 推导，不能依赖 runner scratch `HOME`；文本枚举使用 macOS 可用的 `grep --null` 与 `read -d ''` 配对，禁止会输出换行却被 NUL reader 忽略的 `grep -Z`。这些位置均由 V3 精确证据定位。
...
- 修复构建时 relocation/sanitization 后，以完整机器证明收口：
  `"$MEGASTORM_TEST_PYTHON" -m pytest tests/test_packaging.py && scripts/build-app.sh --output dist && scripts/verify-app.sh dist/LocalModelDesk.app`。实现 cherry-pick task commit `7d2e554`，保留其离线 runtime、bytecode、relocation、signing 与内部 verify 修复；只把错误写入源码的 Perl 双反斜杠 `s/\\Q$ENV{FORBIDDEN}\\E//g` 修为 Perl quote-meta 所需的单反斜杠 `s/\Q$ENV{FORBIDDEN}\E//g`，并以精确静态断言防回归。不得重写 sanitizer 或其他构建逻辑。真实 Gatekeeper/安装由 T-packaging-13 reality gate 收口。
