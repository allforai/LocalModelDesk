...
| 首运页无「返回台面」、收编框预填扫描路径 | 待产品决定：D 计划未含；设置里的「重新设置模型目录」已加确认弹层（A-Task 7b） |
| 隔离数据根时 `discovered` 仍扫真实 `$HOME` | 待产品决定：对真实用户无影响，只影响测试隔离 |
| 视频长参数耗时预期 | 待产品决定：缺可靠的每步耗时数据，先不在界面上给数字 |
| VPN 隧道地址时 base URL 选哪个 | 未实证：取证环境无隧道，0908 条件未复现 |
| 菜单栏图标辨识度、状态滞后约 2 秒 | 待产品决定：属视觉/轮询策略取舍 |
| 作业列 2560 宽时视频预览贴左 | 不改：基线 L3 只约束两列之外的空区 |
| 原生窗口内容区 832 与 WebView 696 | 未实证：差值等于标题栏与状态栏高度之和的可能性大，需再取证 |
